...
namespace SchemaDiff.Library.Models
{
    public class Routine : INameable, IHasOwner, IScriptable
    {
        ...
        private const string SqlCreateWithNameRegex =
            SqlCreateRegex + @"+{0}" + Database.SqlWhitespaceOrCommentRegex + @"+?(?:(?:(" +
        ...
        private string ScriptBase(Database db, string definition)
        {
            ...
            definition = string.IsNullOrEmpty(definition)
                ? $"/* missing definition for {RoutineType} [{Owner}].[{Name}] */"
                : RemoveExtraNewLines(definition);
