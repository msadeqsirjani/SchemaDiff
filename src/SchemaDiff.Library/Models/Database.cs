using SchemaDiff.Library.Models.Compares;
using System;
...
namespace SchemaDiff.Library.Models
{
    public class Database
    {
        ...
        {
            Props.Add(new DbProperty("COMPATIBILITY_LEVEL", ""));
            Props.Add(new DbProperty("COLLATE", ""));
            Props.Add(new DbProperty("AUTO_CLOSE", ""));
            Props.Add(new DbProperty("AUTO_SHRINK", ""));
            Props.Add(new DbProperty("ALLOW_SNAPSHOT_ISOLATION", ""));
            Props.Add(new DbProperty("READ_COMMITTED_SNAPSHOT", ""));
            Props.Add(new DbProperty("RECOVERY", ""));
            Props.Add(new DbProperty("PAGE_VERIFY", ""));
            Props.Add(new DbProperty("AUTO_CREATE_STATISTICS", ""));
            Props.Add(new DbProperty("AUTO_UPDATE_STATISTICS", ""));
            Props.Add(new DbProperty("AUTO_UPDATE_STATISTICS_ASYNC", ""));
            Props.Add(new DbProperty("ANSI_NULL_DEFAULT", ""));
            Props.Add(new DbProperty("ANSI_NULLS", ""));
            Props.Add(new DbProperty("ANSI_PADDING", ""));
            Props.Add(new DbProperty("ANSI_WARNINGS", ""));
            Props.Add(new DbProperty("ARITHABORT", ""));
            Props.Add(new DbProperty("CONCAT_NULL_YIELDS_NULL", ""));
            Props.Add(new DbProperty("NUMERIC_ROUNDABORT", ""));
            Props.Add(new DbProperty("QUOTED_IDENTIFIER", ""));
            Props.Add(new DbProperty("RECURSIVE_TRIGGERS", ""));
            Props.Add(new DbProperty("CURSOR_CLOSE_ON_COMMIT", ""));
            Props.Add(new DbProperty("CURSOR_DEFAULT", ""));
            Props.Add(new DbProperty("TRUSTWORTHY", ""));
            Props.Add(new DbProperty("DB_CHAINING", ""));
            Props.Add(new DbProperty("PARAMETERIZATION", ""));
            Props.Add(new DbProperty("DATE_CORRELATION_OPTIMIZATION", ""));
        ...
        public List<DbProperty> Props { get; set; } = new();
        ...
        public DbProperty FindProp(string name)
        {
        ...
        private void LoadSynonyms(SqlCommand cm)
        {
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    { BaseObjectName = (string)dr["base_object_name"] };
...
";
        ...
        private void LoadUsersAndLogins(SqlCommand cm)
        {
            ...
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    if (u != null && dr["password_hash"] is not DBNull)
        ...
        private void LoadXmlSchemas(SqlCommand cm)
        {
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    {
                        Text =
                            $"CREATE XML SCHEMA COLLECTION {dr["DBSchemaName"]}.{dr["XMLSchemaCollectionName"]} AS N'{dr["definition"]}'",
        ...
        private void LoadRoutines(SqlCommand cm)
        {
            ...
            while (dr.Read())
            {
                ...
                {
                    Text = dr["definition"] is DBNull ? string.Empty : (string)dr["definition"],
            ...
            while (dr.Read())
            {
                ...
                catch (Exception ex)
                {
                    throw new ApplicationException(
                        $"{dr["TABLE_SCHEMA"]}.{dr["TABLE_NAME"]} : {ex.Message}", ex);
                }
            }
        }
...
";
        ...
        private static void LoadUserDefinedTypesBase(IDataReader dr,
            ICollection<UserDefinedType> userDefinedTypes)
        {
            while (dr.Read())
            {
        ...
        private static void LoadTablesBase(IDataReader dr, bool areTableTypes,
...
";
        ...
        private static string MakeFileName(object o)
        {
            while (true)
            {
                switch (o)
                {
                    ...
                    case Constraint { Type: "CHECK" } constraint:
                        o = constraint.Table;
                        continue;
                }

                var schema = o is not IHasOwner owner ? "" : owner.Owner;
                var name = o is not INameable nameable ? "" : nameable.Name;

                var fileName = MakeFileName(schema, name);

                // prefix user defined types with TYPE_
                var prefix = o is not Table table ? "" : table.IsType ? "TYPE_" : "";
        ...
        public static string ScriptPropList(IList<DbProperty> props)
        {
            var text = new StringBuilder();
            ...
            Action<TraceLevel, string> log = null)
        {
            ...
            var exception = new BatchSqlFileException
            {
                Exceptions = errors
            };
            throw exception;
    ...
    public class DatabaseDiff
    {
        ...
        public List<DbProperty> PropsChanged = new();
